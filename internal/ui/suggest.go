        ...
        case tea.KeyMsg:
                ...
                if m.editing {
                        ...
                        case key.Matches(msg, m.keys.Back):
                                ...
                                // Reset the list index to match the current detail index
                                if len(m.suggestions) > 0 && m.currentDetailIndex < len(m.suggestions) {
                                        m.list.Select(m.currentDetailIndex)
                                }
                                return m, nil
...
func (m SuggestModel) View() string {
        ...
        if len(m.suggestions) == 0 {
                ...
                visibleCount := 5 // Number of items to show at once
                startIdx := m.list.Index() - (visibleCount / 2)
                if startIdx < 0 {
                    startIdx = 0
                }
                ...
                if startIdx > 0 {
                    content = append(content, styleListItem.Copy().Foreground(dimmedColor).Render("↑ More suggestions above"))
                }
                ...
                for i := startIdx; i < endIdx; i++ {
                        ...
                        if i == m.list.Index() {
                                fileStyle = fileStyle.Foreground(dimmedColor)
                        }
                ...
                // Show scroll indicators if needed
                if endIdx < len(m.suggestions) {
                    content = append(content, styleListItem.Copy().Foreground(dimmedColor).Render("↓ More suggestions below"))
                }
        }
...
func (m SuggestModel) renderDetailView() string {
        ...
        if len(msgParts) > 1 {
                ...
                for _, line := range bodyLines {
                        ...
                        if trimmedLine != "" && !seenLines[trimmedLine] {
